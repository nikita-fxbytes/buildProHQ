...
export class TasksAnalyticsService {
  ...
  async getStats(
    ...
    if (filters?.projectIds?.length) {
      qbAll.andWhere('t.project_id IN (:...projectIds)', {
    ...
    if (term) {
      qbAll.andWhere(
        '(t.title ILIKE :search OR t.description ILIKE :search OR uc.full_name ILIKE :search OR uc.initials ILIKE :search OR p.name ILIKE :search)',
        { search: `%${term}%` },
      );
    }
